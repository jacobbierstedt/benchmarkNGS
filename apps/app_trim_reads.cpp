...
static int minQEnd            = 20;
static bool enableQual        = true;
static int windowSize         = 20;
static int windowMinQual      = 20;
static int meanQual           = 30;
...
void usage(int xc) {
  ...
  fprintf(stderr, "\nRead QC options:\n");
  fprintf(stderr, "-x          disable quality trimming and filtering\n");
  fprintf(stderr, "-e int      Q-score threshold at ends of reads (Default: 20)\n");
  fprintf(stderr, "-w int      sliding window size (Default: 20)\n");
  fprintf(stderr, "-u int      sliding window quality threshold (Default: 20)\n");
  fprintf(stderr, "-a int      mean quality threshold, discard reads below (Default: 30)\n");
...
void parse_command_line(int argc, char ** argv) {
  int c;
  while ((c = getopt(argc, argv, "hl:t:f:m:p:dz:xe:w:u:a:i:o:")) != -1) {
    switch(c) {
      ...
      case 'x': enableQual      = false;               break;
      case 'e': minQEnd         = atoi(optarg);        break;
      case 'w': windowSize      = atoi(optarg);        break;
      case 'u': windowMinQual   = atoi(optarg);        break;
      case 'a': meanQual        = atoi(optarg);        break;
